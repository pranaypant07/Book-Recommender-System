...
from mongodb_connection import get_books_collection

stop_list = set(stopwords.words('english'))


features_df = pd.read_csv('H:/consolidate/input_files/omni_books_data.csv')
...
for i in range(0, len(features_df)):
    ...
    series = features_df.iloc[i, 1]
    author = features_df.iloc[i, 2]
    bisac = features_df.iloc[i, 5]
    audience = features_df.iloc[i, 6]
    annotation = features_df.iloc[i, 7]
...
# authorsim_general = cosine_similarity(authormatrix_general, authormatrix_general)
...
def get_top_books_general():
    combinedsim = (annotationssim_general + bisacsim_general + audiencesim_general + seriessim_general + authorssim_general) / 5
    s = np.argsort(combinedsim, axis =1)
    top_books = {}
    for i in range(len(isbn_general)):
        x = s[i][-100:]
        test = []
        for j in x:
            test.append(str(isbn_general[j]))
        top_books[str(isbn_general[i])] = test
    return top_books


def get_top_books_teen():
    combinedsim = (annotationssim_teen + bisacsim_teen + audiencesim_teen + seriessim_teen + authorssim_teen) / 5
    s = np.argsort(combinedsim, axis =1)
    top_books = {}
    for i in range(len(isbn_teen)):
        x = s[i][-100:]
        test = []
        for j in x:
            test.append(str(isbn_teen[j]))
        top_books[str(isbn_teen[i])] = test
    return top_books

def get_top_books_scholarly():
    combinedsim = (annotationssim_scholarly + bisacsim_scholarly + audiencesim_scholarly + seriessim_scholarly + authorssim_scholarly) / 5
    s = np.argsort(combinedsim, axis =1)
    top_books = {}
    for i in range(len(isbn_scholarly)):
        x = s[i][-100:]
        test = []
        for j in x:
            test.append(str(isbn_scholarly[j]))
        top_books[str(isbn_scholarly[i])] = test
    return top_books

def get_top_books_children():
    combinedsim = (annotationssim_children + bisacsim_children + audiencesim_children + seriessim_children + authorssim_children) / 5
    s = np.argsort(combinedsim, axis =1)
    top_books = {}
    for i in range(len(isbn_children)):
        x = s[i][-100:]
        test = []
        for j in x:
            test.append(str(isbn_children[j]))
        top_books[str(isbn_children[i])] = test
    return top_books
...
if __name__=='__main__':
    pool = Pool()
    # general_dict = pool.apply_async(get_top_books_general)
    teen_dict = pool.apply_async(get_top_books_teen)
    scholarly_dict = pool.apply_async(get_top_books_scholarly)
    children_dict = pool.apply_async(get_top_books_children)

    merge_dict_one = {**teen_dict.get(),**scholarly_dict.get()}
    merge_dict_two = {**merge_dict_one, **children_dict.get()}

    merge_dict = {**merge_dict_one,**merge_dict_two}
    # merge_dict = get_top_books_scholarly()
    collection = get_books_collection()
    collection.insert(merge_dict)
